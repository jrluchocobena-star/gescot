...
if ($accion=="mostrar_datos_contacto"){	
	...
	*/
	...
	while($reg_lisT=mysql_fetch_row($res_lis)){		
		...
		<!--
        <a href="javascript:editar_contacto('<? echo $reg_lisT[0] ?>','<? echo $reg_lisT[2] ?>','<? echo $reg_lisT[3] ?>','<? echo $reg_lisT[4] ?>',
        '<? echo $reg_lisT[6] ?>','<? echo $reg_lisT[8] ?>','<? echo $reg_lisT[10] ?>','<? echo $reg_lisT[5] ?>','<? echo $reg_lisT[7]?>',
        '<? echo $reg_lisT[9]?>','<? echo $reg_lisT[11]?>');">
       
         -->
        <a href="javascript:popup_reclamo('2','<? echo $reg_lisT[0] ?>','<? echo $reg_lisT[2] ?>','<? echo $iduser; ?>','','');">
        
		<? echo "$reg_lisT[2]</a></td>";	
...
echo "<tr>";
echo "<td colspan='6' class='caja_sb'>Sr(a). <? echo $usu[1]; ?> se le asignado el Pedido Nro. <? echo $reg[1]; ?>. Ud. es el encargado de darle tratamiento y poder dar una gestion optima y personalizada del pedido asignado.  Dar click en el numero de pedido para hacer la gestión respectiva.
    <p> 
  ...
  */
  ...
  while($reg_lis=mysql_fetch_row($res_lis_2)){		 			
        ...
        onclick="javascript:popup_reclamo('1','<? echo $reg_lis[0]?>','<? echo $reg_lis[4]?>','<? echo $iduser;?>','<? echo $reg_lis[5]?>')" border="1" />	
        <?
		echo $reg_lis[1];		
        echo "</td>";		
...
$res_6= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO MULTICONSULTA','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");
$res_7= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO INTRAWAY','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO'");
$res_8= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO WEB UNIFICADA','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");		
$res_9= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO PSI','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");					$res_10= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO ATIS','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_11= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO CMS','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_12= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO GESTEL','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_13= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO WEB INCIDENCIAS PSI','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_14= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO PDM','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_15= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO CLEAR VIEW','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_16= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO WEB SARA','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_17= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO WEB ASEGURAMIENTO','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_18= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO ARPU CALCULADORA','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_19= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO GENIO','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_20= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO WEB ASIGNACIONES','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");				$res_21= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO WEB SIGTP MAPA GIG','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");	
$res_22= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO RED','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");	
$res_23= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO GESCOT','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");	
$res_24= mysql_query("insert into movimientos_maestra VALUES('','$dni','','USUARIO GENESYS','ACT. USUARIO',NOW(),CURDATE(),'','$iduser','registrado','ACTIVO')");			
...
if ($accion=="actualizar_datos_usu_horiz"){
	...
	*/
	...
	if ($_GET["supervisor"]<>"0"){
		$dato=$supervisor;
		$tarea="CAMBIO DE SUPERVISOR";		
	}else{
		if ($_GET["monitor"]<>"0"){
		$dato=$monitor;	
		$tarea="CAMBIO DE MONITOR";		
		}	
	}
...
if ($accion=="carga_combo"){
	...
	<td>";
	?>
	<select name="combo_mot" id="combo_mot" class="caja_sb" onchange="javascript:mostrar_modo2(this.value)" >
          <? 			
...
if ($accion=="grabar_incidencia_prueba"){
	...
	for ($con = 0 ; $con < $nf_tiempo  + 1 ; $con++) {
		...
		$sql_1="INSERT INTO cab_incidencia_
		...
		$cad=" SELECT LAST_INSERT_ID() from cab_incidencia_";
		$rs = mysql_query($cad);
		$rg = mysql_fetch_array($rs);
		$idfranqueo="INC-".substr($dia,0,4).substr($dia,5,2).substr($dia,8,2).str_pad($rg[0], 4, '0', STR_PAD_LEFT);
		//echo $idfranqueo; 	
		
		$cad_1="update cab_incidencia_ set cod_incidencia='$idfranqueo' where id='$rg[0]'";
		echo $cad_1;
...
$connection_teradata 	= db_conn_teradata();
	...
	/*
	$sql0	="SELECT * FROM movimientos_maestra WHERE dato='$xusu_mov' and dni='$xdni_mov'";
	//echo $sql0;
	...
	if ($nrg > 0){
		echo "USUARIO $xusu_mov YA SE ENCUENTRA REGISTRADO CON OTRO DNI";	
	}else{
		...
		$sql2="insert into movimientos_maestra values('','$xdni_mov','$xusu_mov','$xapli_mov','ACT. USUARIO',now(),'$xfec_mov','2020-01-01','$iduser','','ACTIVO')";
		//echo "<br>".$sql2;
	...
	*/
	/*hecho por luis saldarriaga */
	$cad_tdata="insert into dbi_cot.ls_movimientos_maestra values('','$xdni_mov','$xusu_mov','$xapli_mov','ACT. USUARIO',now(),'$xfec_mov','2050-01-01','$iduser','','ACTIVO')";
