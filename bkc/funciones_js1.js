...
function aprobar_incidencias_sup(estado) {
 ...
 console.log("estado",estado);
    if (estado === "2" || estado == 2) {
        ...
        motivoRechazoIncidencia(inc_escogidas, nro_escogidos, iduser, idperfil, estado);
        return;
    } else {
        if (nro_escogidos == "") {
            alert("No a escogido valores de la lista");
...
function obtenerMinutosHVi() {
	...
	const h_verano = ["01", "02", "03"]; // Enero, Febrero, Marzo
	let minutos = 0;
...
function validarHorarioGestorIncidencia(horainicio, minutoinicio, horafin, minutofin) {
	...
	const hsalida = ${horafin}:${minutoinicio};
...
function grabar_incidencias_cot_nuevo(){
    ...
    if (!validarRangoSemana()) {
        alert("Error: No se selecciono una fecha valida.");
        console.log('error');
        return;
    }
    if (!validarMinutosIncidencia(hinicio, minicio, hfinal, mfinal)) {
        alert("Error: No se selecciono un horario valido.");
        console.log('error');
        return;
    }
	...
	if (document.getElementById("obs_gru").value==""){
		...
		if (document.getElementById("tp_incidencia").value=="0"){
			alert("Error: Tipo de incidencia sin escoger")	
			return
		}else{
			if (document.getElementById("modalidad").value=="0"){
			alert("Error: Tipo de modalidad sin escoger")	
			return
		}else{		
					if (document.getElementById("modalidad").value=="D"){
						var fec1 = document.getElementById("inputg1").value;	
