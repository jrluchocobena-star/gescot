...
function menu(opc){	
	...
	if (iduser=="" || iduser==" " || iduser=="undefined"){
		...
		if (opc=="17"){		 
			 var pag="ws_registrar_tecnicos.php?iduser="+iduser+"&idperfil="+idperfil+"&pc="+pc;			 
		}
...
function registrar_usuario(){	
	...
	+"&cip="+cip+"&grupo="+grupo+"&fec_nac="+fec_nac+"&ncompleto="+ncompleto+"&login="+login+"&ola="+ola+"&accion=registrar_usuario";
	//alert(pag2)	
	ajaxc1 = createRequest();
    ajaxc1.open("get", pag2, true);
    ajaxc1.onreadystatechange = function () {
...
function  popup_reclamo(tipo,val1,val2,val3,val4,val5){	
	...
	'width=1500px,height=800px,top=10px,left=50px,resize=0,scrolling=1');	
	}
...
function buscar_contacto(){
	...
	var ncontacto = document.getElementById("ncontacto").value;	
	var pedido = document.getElementById("pedido").value;	
	document.getElementById("bdatos").value="GESCOT";
	//var iframe = document.getElementById("f_inferior");
	document.getElementById("div_lista_contactos").style.display="none"; 
	document.getElementById("d_historico_contactos").style.display="none"; 
...
function ventana_modal(opcion){
	...
	var page		="bandeja_incidencias_cot_supervisores.php?iduser="+iduser+"&idperfil="+idperfil;
	var atributos 	="width=1500px,height=700px,top=20px,left=50px,center=0,resize=0,scrolling=0";
	...
	var page		="modulo_compensacion_v1.php?iduser="+iduser+"&idperfil="+idperfil+"&dni="+dni+"&dni="+dni;
	var atributos 	="width=1200px,height=500px,center=1,resize=0,scrolling=0";
	...
	return	
		...
		ajaxc.onreadystatechange = function () {
			...
			if (ajaxc.readyState == 4) {  
				...
				if (document.getElementById("opc").value=="1"){
						document.getElementById("c_supervisor").style.display	= "none ";
				}
				if (document.getElementById("opc").value=="2"){
						document.getElementById("c_monitor").style.display		= "none ";
				}
				if (document.getElementById("opc").value=="3"){
						document.getElementById("c_local").style.display		= "none ";
				}
...
function ver_div_incidencias(){
	...
	var pag_inc = "grid_incidencias.php?iduser="+iduser+"&idperfil="+idperfil+"&u_reg="+u_reg+"&c_inc="+c_inc+"&gestor="+gestor+"&fec_i="+fec_i+"&fec_f="+fec_f;		
...
function ver_incidencia(c0,c1,c2,c3,c4,c5,c6,c7,c8){
	...
	parent.document.getElementById("x_fec2").value=c3;
	parent.document.getElementById("x_obs").value=c4;
	parent.document.getElementById("x_registrado").value="-";	
	}
...
function reporte_inc(opc){
	...
	if (opc=="1"){
	var url = "reporte_incidencias_nuevo_v1.php?iduser="+iduser+"&idperfil="+idperfil+"&u_reg="+u_reg+"&c_inc="+c_inc+"&gestor="+gestor+"&fec_i="+fec_i+"&fec_f="+fec_f;			
	}
...
function buscar_contacto_tdata(){
	...
	if (document.getElementById("usu_contactos").value==""){
		...
		var pag2 = "funciones_cot.php?dni="+dni+"&idperfil="+idperfil+"&pc_asig="+pc_asig+"&pedido="+pedido
		+"&ncontacto="+ncontacto+"&ape_pat="+ape_pat+"&bd="+bd+"&ape_mat="+ape_mat+"&accion=validar_contacto_tdata";
...
function historico_contactos_tdata(){		
	...
	//.document.getElementById("apuntado").focus();
	//win=window.open(url,true);	
...
function buscar_reniec() {	
	...
	var dni=document.getElementById("dni").value;
	var iduser=document.getElementById("iduser").value;
	var accion = "detalle_maestra_reniec"; 
...
function grabar_movimientos_usuarios() {	
	...
	if (xid_ant=="" || xusu_mov=="" || xfec_mov=="0000-00-00"){
		alert("Datos Importantes Vacios")
		return
	}else{
...
function grabar_incidencia_masiva(){
	...
	var c_mot_mas = "INCIDENCIAS DE SISTEMAS";	
	var c_mot_inc_mas = "600";	
	...
	if (document.getElementById("c_incidencia_mas").value=="" || document.getElementById("c_incidencia_mas").value=="INC-1"){
		alert("EL CODIGO DE INCIDENCIA ESTA VACIO O TIENE UN DATO ERRONEO(INC-1)")	
		return	
	}else{
		...
		var pag="funciones_cot.php?iduser="+iduser+"&dni_escogidos="+dni_escogidos+"&c_incidencia="+c_incidencia+"&c_mot_mas="+c_mot_mas
		+"&c_mot_inc_mas="+c_mot_inc_mas+"&modo_mas="+modo_mas+"&tiempo_mas="+tiempo_mas+"&obs_mas="+obs_mas+"&c_supervisor="+c_supervisor
		+"&fec_ini="+xfec_ini+"&fec_fin="+xfec_fin+"&c_doid_mas="+c_doid_mas+"&nro_escogidos="+nro_escogidos+"&accion=grabar_incidencia_masiva";
		//alert(pag)
...
function mostrar_bandeja_gestores(valor){
		...
		var pag = "funciones_cot.php?valor="+valor+"&accion=listar_bandeja_gestores";
...
function msn_alerta(modulo,valor_1,valor_2){	
	...
	if (modulo=="1"){ // registro de boleta
		...
		var p_modulo="alertas.php?texto_ini="+texto_ini+"&texto_int="+texto_int+"&valor_2="+texto_fin+"&iduser="+iduser+"&modo="+modo
		+"&idperfil="+idperfil+"&estado="+estado+"&dni="+dni;
		//alert(p_modulo);	
		
		var atributos = "width=800px,height=400px,center=1,resize=0,scrolling=0";
...
function carga_combo_modo(nom_combo,valor1){		
		...
		var urlx = "funciones_cot.php?valor1="+valor1+"&combo="+nom_combo+"&accion=carga_combo_modo";	
...
function grabar_compensaciones_inc(){
		var dni = document.getElementById("dni").value;	
		var c_incidencia = document.getElementById("c_incidencia").value;			
		var iduser = document.getElementById("iduser").value;		
		var modo = document.getElementById("modalidad").value;
		...
		if (fec_ini_comp <= fec_fin_inc){
					alert("La fecha de compensacion debe ser mayor que la fecha de permiso")
					return
			
		}else{
			if (tt_acu_comp >= tt_acumulado_perm){
				alert("El tiempo Programado no debe ser mayor que el tiempo acumulado del permiso")
				return
			}else{
		...
		+"&xfec_fin_inc="+xfec_fin_inc+"&modo="+modo+"&c_incidencia="+c_incidencia+"&hor_ini="+h_ini+"&h_fin="+h_fin
		+"&iduser="+iduser+"&tt_acu_comp="+tt_acu_comp+"&tt_acumulado_perm="+tt_acumulado_perm+"&accion=save_compensaciones_inc";
...
function act_contacto(dni,telefono,v_combo){		
	var iduser = document.getElementById("iduser").value;
	var accion ="act_contacto_tdata" ;
	
	var msn="Desea actualizar informacion..?";
	if (confirm(msn)){			
		var url = "funciones_cot.php?dni="+dni+"&iduser="+iduser+"&telefono="+telefono+"&v_combo="+v_combo+"&accion="+accion;
