...
$lista="(select *,SEC_TO_TIME((TIMESTAMPDIFF(MINUTE ,fec_ini_inc, fec_fin_inc))*60) AS diferencia,
	...
	from cab_incidencia WHERE $cad and tp_incidencia='INCIDENCIAS DE SISTEMAS'
	order by fec_reg desc)union(select *,SEC_TO_TIME((TIMESTAMPDIFF(MINUTE ,fec_ini_inc, fec_fin_inc))*60) AS diferencia,
	SUBSTR(fec_ini_inc,1,7)  
	from cab_incidencia_2018 WHERE $cad and tp_incidencia='INCIDENCIAS DE SISTEMAS'
...
$i=0;
	...
	echo "<table width='100%' border='1'  style='border-collapse:collapse'>";		
			...
			echo "<td class='fdotxttabla2'>NRO.DOID</td>";		
