...
for ($sw=1;$sw<6;$sw++){
	...
	if ($sw==2){
		Carga_pedidos_cms();		
		//echo "<br>"."proceso 2 terminado";		
		$sw=3;
	}
	
	if ($sw==3){		
		Carga_pedidos_gestel_423();		
		$sw=4;
...
function Carga_pedidos_cms(){
		$ini1=date("Y-m-d H:m");
		
//		$sw="1";	
		//rename('d:/data_cot/cms/Lima.11.20AM.xls', 'd:/data_cot/cms/Lima_1120AM.csv');
		
		$base = "d:/data_cot/cms/Lima_1120AM.csv" ;
		$tabla = "tb_cms";			
		
		
		$qry_0 = "LOAD DATA LOCAL INFILE '$tabla' IGNORE FIELDS TERMINATED BY ';' ENCLOSED BY ''  TERMINATES BY '' IGNORE 1 LINES"; 	
		//echo "<br>Carga_pedidos_cms ".$qry_0;		
		$res_0 = mysql_query($qry_0) or die(mysql_error($qry_0));	

		archivo_log($ini1,'USER_WIN','CARGA GESTEL 423','1.CARGA PEDIDOS CMS');			
}


function Carga_pedidos_gestel_423(){	
...
function juntar_2tablas(){	

$ini3=date("Y-m-d H:m");
$tabla = "tb_gestel_423";

$sql_3="INSERT INTO carga_pedidos_total
...
FROM tb_cms)";
echo "<BR>".$sql_3;
$res_sql3 = mysql_query($sql_3) or die(mysql_error());	

$sql_4="UPDATE carga_pedidos_total a, cab_asignaciones_cot b
SET estado='3' 
WHERE a.pedido=b.pedido";
//echo "<BR>".$sql_3;
$res_sql4 = mysql_query($sql_4) or die(mysql_error());	

$sql_5="UPDATE carga_pedidos_total a,  tb_gestel_423_prov b
SET a.zonal=b.zonal
WHERE a.pedido=b.pedido";
//echo "<BR>".$sql_3;
$res_sql5 = mysql_query($sql_5) or die(mysql_error());	


$sql_6="UPDATE carga_pedidos_total SET zonal='LIM' WHERE zonal=''";
//echo "<BR>".$sql_3;
$res_sql6 = mysql_query($sql_6) or die(mysql_error());	
