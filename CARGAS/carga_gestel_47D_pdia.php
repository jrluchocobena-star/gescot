...
//$fileName = "d:/data_cot/Reasignaciones_47D/GESTEL_47D_LIM.txt";	
	...
	//echo "<br>"."Entro";
		...
		carga_tabla_GESTEL_47D($fileName,$zon,$dia);
...
function carga_tabla_GESTEL_47D($fileName,$zon,$dia){	
	...
	while (!feof($gestel_47D) ) {
		...
		*/
		...
		$sql_1="INSERT INTO $tabla (Fecha,Comprob_,Inscripcion,Solicitud,Usuario,Cod_Sol,Motivo,Pedido,Red,MDF,Cabl ,PAli ,Armar ,Bloq ,PDis ,Caja ,Born ,MDFd,Cabld ,PAlid ,Armard ,Bloqd ,PDisd ,Cajad ,Bornd ,Telefono ,Sect ,Manz,Peticion ,Agrupacion ,Cliente,Cuenta ,PC,Tipo_Circuito1,N_Circuito1,Tipo_Circuito2,N_Circuito2,Segmento,origen,fec_carga,n_area) 
			...
			'$Telefono','$Sect','$Manz','$Peticion','$Agrupacion','$Cliente','$Cuenta','','','','','','','$zon','$dia','')";
	...
	$sql_2="DELETE FROM $tabla WHERE SUBSTR(fecha,1,1) IN ('','-','f','t','x','s')";		
		//echo "<br>".$sql;
	$res_2 = mysql_query($sql_2) or die(mysql_error());	
		
	$sql_3="DELETE FROM $tabla WHERE SUBSTR(fecha,1,4)!='2018'";		
		//echo "<br>".$sql;
	...
	*/
	...
	$sql_4="DELETE FROM gestel_47d_total WHERE SUBSTR(fec_carga,1,10)='$dia'";			
		//echo "<br>".$sql;
	$res_4 = mysql_query($sql_4) or die(mysql_error());



	$sql_5="INSERT INTO gestel_47d_total
		SELECT * FROM $tabla";			
		//echo "<br>".$sql;
