 ...
 *
 * This library is free software; you can redistribute it or/or
...
/* hide popup or cleanup */
AC.prototype.close_popup = function() { 
	this.div.style.visibility = 'hidden'; 

	/* no selected item, no typing, or close any pending request */
...
function _ac_key_thunk_call(id) {
	if (_ac_key_thunk[id]) {
		...
		if (ac.poll_input) {
			...
			/* remove from list or cleanup list */
...
function _ac_key_check(ac,timeout) {
	...
	var horle = setTimeout("_ac_key_thunk_call("+i+")",timeout);
	_ac_key_thunk[i] = new Array(ac.obj.id,ac,horle);
}
...
function _ac_add(ac,query,timeout) {
	...
	var horle = setTimeout("_ac_thunk_call("+i+")",timeout);
	_ac_thunk[i] = new Array(ac.obj.id,ac,query,horle);
}
...
AC.prototype.send = function(query) {
	...
	if (this.iframe == null) {
		...
		if (this.request == null) {
			...
			iframe.src = this.url+'?i=1&id='+encodeURIComponent(this.obj.id)+'&t='+encodeURIComponent(this.type)+'&s='+encodeURIComponent(query)+"&aleatorio="+aleatorio;
			...
			/* Aqui podemos capturar lo que envia la funcion de Ajax al archivo php
			para consultar los querys.
			Ejemplo:
				alert("hola : "+this.url+"?id="+encodeURIComponent(this.obj.id)+"&t="+encodeURIComponent(this.type)+"&s="+encodeURIComponent(query)); 
			/*Ricardo de la Torre  */
				//Aado este If solo para el caso de los suggest de liquidacion

				/*if(encodeURIComponent(this.obj.id=='motivo1')){
						var solucion=document.getElementById('categoria').value;
						var valor=document.getElementById('categoria1').value;
						var cod_categoria=valor.substring(0,2);
				
						this.request.open("GET", this.url+"?id="+encodeURIComponent(this.obj.id)+"&t="+encodeURIComponent(this.type)+"&s="+encodeURIComponent(query)+"&cod_categoria="+cod_categoria+"&solucion="+solucion+"&aleatorio="+aleatorio);						
						this.request.send(null);
					
				}else{*/
				//alert("hola : "+this.url+"?id="+encodeURIComponent(this.obj.id)+"&t="+encodeURIComponent(this.type)+"&s="+encodeURIComponent(query)); 		
					this.request.open("GET", this.url+"?id="+encodeURIComponent(this.obj.id)+"&t="+encodeURIComponent(this.type)+"&s="+encodeURIComponent(query))+"&aleatorio="+aleatorio;
		...
		/* re-submit iframe */
		this.iframe.src = this.url+'?i=1&id='+encodeURIComponent(this.obj.id)+'&t='+encodeURIComponent(this.type)+'&s='+encodeURIComponent(query)+"&aleatorio="+aleatorio;
		this.obj.focus();
	}
}
