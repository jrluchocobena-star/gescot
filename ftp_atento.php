...
/**
* Directorios Locales donde se copiaran los archivos
*/
$src_dir='D:/cot';
$tmp_dir='D:/COMPARTIDO/DATA/CANCELADAS_ROBOT';
...
foreach ($files as $file) {
	...
	if(substr($filename,0,18) == $file_comercial){
		if($listing->downloadFile($filename,$src_dir."/".$filename))
		{
			...
			copy($src_dir."/".$filename,$tmp_dir."/".$filename);
...
/**
* Obtiene archivos de ftp://10.4.40.49/canceladas/robot manual/
*/
$new_path = '/usrftpcot/canceladas/Robot manual';
$new_server = new TmFtpServer($servername, $user, $pass, $new_path);
$new_listing = new TmFtpServerWinFileListing($new_server);
$new_files = $new_listing->setDirectoriesToSkip($directories_to_skip)
		->setFilePatternsToSkip($file_patterns_to_skip)
		->getFileList($depth_max, $show_directories);

$file_TecnicasInput='TecnicasInput_'.date('Ymd');
foreach ($new_files as $new_file) {
	$filename = $new_file->name;
	if(substr($filename,0,22) == $file_TecnicasInput){
		if($new_listing->downloadFile($filename,$tmp_dir."/".$filename))
		{
			print "Archivo ".$filename." descargado a PC \n";
			fwrite($gestor,"Archivo ".$filename." descargado a PC \r\n");
		}
