...
$xreporte=$_GET["xreporte"];

if ($xreporte=="423"){
$xmes=$_GET["xmes"]."/".date("y");
$archivo_out="D://xampp/htdocs/cot/REPORTES/gestel_423_total_".$_GET["xmes"].date("y").".csv";

$sql="(select 'CIUDAD','PEDIDO','INSCRIPCION','DIRECCION','DISTRITO','PROMOCION','GRUPO','NRO_REF','FECHA_REG','TIPO_SERVICIO','PRIORIDAD','OFIC_COM','PETICION',
...
union(SELECT CIUDAD,PEDIDO,INSCRIPCION,DIRECCION,DISTRITO,PROMOCION,GRUPO,NRO_REF,FECHA_REG,TIPO_SERVICIO,PRIORIDAD,OFIC_COM,PETICION, AGRUPACION,CLIENTE,CUENTA,PC,NEGOCIO,PREFIJO,SEGMENTO,CANTIDAD,SUB_SEGMENTO,IRIS,DIRECCION_ANTIGUA,SECTOR,MANZANA,COORD_X,COORD_Y  
FROM tb_gestel_423_total WHERE SUBSTR(FECHA_REG,4,5)='$xmes' 
...
$archivo_out="D://xampp/htdocs/cot/REPORTES/gestel_47D_total_".$_GET["xmes"].date("y").".csv";
$xmes=date("Y")."-".$_GET["xmes"];	
	
$sql="(select 'Fecha','Comprob_','Inscripcion','Solicitud','Usuario','Cod_Sol','Motivo','Pedido','Red','MDF','Cabl','PAli',
'Armar','Bloq','PDis','Caja','Born','MDFd','Cabld','PAlid','Armard','Bloqd','PDisd','Cajad','Bornd','Telefono','Sect','Manz','Peticion',
'Agrupacion','Cliente','Cuenta','PC','Origen','Area')
union(SELECT Fecha,Comprob_,Inscripcion,Solicitud,Usuario,Cod_Sol,Motivo,Pedido,Red,MDF,Cabl,PAli,Armar,Bloq,PDis,Caja,Born,MDFd,Cabld,PAlid,
Armard,Bloqd,PDisd,Cajad,Bornd,Telefono,Sect,Manz,Peticion,Agrupacion,Cliente,Cuenta,PC,Origen,n_area
FROM gestel_47d_total
WHERE SUBSTR(fecha,1,7)='$xmes'
INTO OUTFILE '$archivo_out'
...
$archivo_out="v://INFORMES/PSI/formato_psi_".date("mY").".csv";
