...
if ($accion=="carga_combo"){
	...
	<td>";
	?>
	<select name="combo_mot" id="combo_mot" class="caja_texto_pe" onchange="javascript:carga_combo_modo('modo_gru',this.value)" >
          <?php 			
	...
	if ($modo==""){
		...
		if ($modo=="0"){ // cuando combo modo esta desabilitado
			...
			if ($modo=="H"){
				...
				$cad_1   = "SELECT SEC_TO_TIME((TIMESTAMPDIFF(MINUTE ,'$fec_ini', '$fec_fin'))*60)";
				//echo $cad;
				$res_1   = mysql_query($cad_1);
				$reg_1   = mysql_fetch_row($res_1);

				$valor1 = limpia_espacios($reg_1[0]);
			
				$cad_2   = "SELECT SUBSTRING_INDEX(SEC_TO_TIME((TIMESTAMPDIFF(MINUTE ,'$fec_ini', '$fec_fin'))*3600),':',1)";
				//echo $cad;
				$res_2   = mysql_query($cad_2);
				$reg_2   = mysql_fetch_row($res_2);
				
				$valor2 = limpia_espacios($reg_2[0]);
...
$lista="select iduser,ncompleto,dni,cip,c_supervisor,c_monitor,local,ola,estado 
from tb_usuarios where estado='HABILITADO'
...
//echo $lista;
	...
	if($idperfil != 1){
		...
		echo "<td class='caja_texto_pekecab'>ESTADO</td>";	
		echo "</tr>";
	$i=0;
	while($reg_lis=mysql_fetch_row($res_lis)){	
	
		$s="select ncompleto from tb_usuarios where iduser='".$reg_lis[4]."'";
		//echo $s;
		$rs_s = mysql_query($s);											
		$rg_s = mysql_fetch_row($rs_s);
		...
		*/
		...
		echo "<td class='caja_texto_peq'>";		// estado		
		echo $reg_lis[8]; 				
		echo "</td>";
...
if ($valor=="0"){
	...
	while($reg_lis=mysql_fetch_row($res_lis)){	
		...
		$s="select * from tb_usuarios where dni='".$reg_lis[17]."'";
		//echo $s;
		$rs_s = mysql_query($s);											
		$rg_s = mysql_fetch_row($rs_s);
		
		$m="select * from tb_motivos_incidencia where cod_mot_inc='".$reg_lis[5]."' AND tp_inc='".$reg_lis[4]."'";
		//echo $s;
		$rs_m = mysql_query($m);											
		$rg_m = mysql_fetch_row($rs_m);
		
		$hor="select * from horarios_gestores_cot where dni='".$reg_lis[17]."'";
		//echo $s;
		$rs_hor = mysql_query($hor);											
		$rg_hor = mysql_fetch_row($rs_hor);
...
if ($accion=="aprobar_incidencias"){
	...
	$nro_escogidos	=$_GET["nro_escogidos"];	
	$idperfil		=$_GET["idperfil"];
    $c_rechazados		=$_GET["c_rechazados"];
	$estado			=$_GET["estado"];
	$motivo_rechazo = isset($_GET["motivo"]) ? $_GET["motivo"] : null;
    $usuario        = "";
	
    if($_GET["estado"]=="1"){
     $ext=", usu_aprobado='$iduser',fec_aprobado=now()"; 
    }
    
    if($_GET["estado"]=="2"){ //rechazo
     $ext=", usu_rechazo='$iduser',fec_rechazo=now(),mot_rechazo='$c_rechazados'"; 
    }
    
    if($_GET["estado"]=="3"){ //cancelado
      $ext=", usu_cancelado='$iduser',fec_cancelado=now(),mot_cancelado='$c_rechazados'";  
    }
	
	for ($x = 1; $x < $nro_escogidos + 1 ; $x++) {	
		$cod = trim($inc_escogidas[$x]);
			
		$cad="update cab_incidencia set estado_inc='$estado'".$ext." where cod_incidencia='$cod'";	
		//echo $cad;	
	...
	/*
	if ($estado == 2) {
        $sql_usu = "SELECT * FROM tb_usuarios WHERE iduser = '$iduser' LIMIT 1";
		echo $sql_usu;
		
        $res_USU = mysql_query($sql_usu);
        if ($res_USU && mysql_num_rows($res_USU) > 0) {
	...
	/*	
    for ($x = 1; $x <= $nro_escogidos; $x++) {
        if (!isset($inc_escogidas[$x])) continue;
        $cod = trim($inc_escogidas[$x]);
        ...
        if ($estado == 2) {
            $cad = "
                UPDATE cab_incidencia
                SET estado_inc = '$estado',
                    usu_mov_est = '$iduser',
                    motivo_rechazo = '" . mysql_real_escape_string($motivo_rechazo) . "',
                    usuario_rechazo = '" . mysql_real_escape_string($usuario) . "',
                    fecha_rechazo = NOW(),
                    fec_mov_est = NOW()
        ...
        $rs_hor = mysql_query($cad) or die("Error actualizando incidencia $cod: " . mysql_error());
...
if ($accion=="mostrar_bandeja_aprobaciones"){
  ...
  */
  			$lista="select * from cab_incidencia where estado_inc='$est'".$ext1.$ext2."
			order by fec_reg desc,fec_mov_est desc";
			...
			if ($nreg>0){	
		 	echo "<table width='80%'>";	
					...
					echo "<td class='caja_texto_db'>FEC.INI.INC</td>";
					echo "<td class='caja_texto_db'>FEC.FIN.INC</td>";
					echo "<td class='caja_texto_db'>TIEMPO</td>";
					echo "<td class='caja_texto_db'>TT EN MIN.</td>";
					echo "<td class='caja_texto_db'>ESTADO</td>";
					echo "<td class='caja_texto_db'>GESTIONADO POR</td>";
					echo "</tr>";
		  ...
		  $i=0;
				...
				while($reg_lis=mysql_fetch_row($res_lis)){	
					...
					$usu_apro="select ncompleto from tb_usuarios where iduser='".$reg_lis[20]."'";
					//echo $s;
					...
					*/
					...
					echo "<td class='caja_texto_peke'>";		//tiempo MINUTOS		
					echo $reg_lis[13]; 		
					echo "</td>";		
...
if ($accion=="grabar_incidencias_cot_oficial") { // esta es la que vale
	...
	if ($reg_valida[0] > 90 ) {   
		$error = "2-Atencion: Solo se puede registrar incidencias con 90 dias de anterioridad";
		echo $error;
	} else {
		...
		if ($c_mot_inc_gru=="41" and $_GET["modo_gru"]=="H") { //Lactancia
					...
					$c_min = "SELECT SUBSTRING_INDEX(SEC_TO_TIME((TIMESTAMPDIFF(MINUTE,'$fecha_hora_inicio', '$fecha_hora_fin'))*3600),':',1)";
					$res_c_min = mysql_query($c_min);
					$reg_c_min = mysql_fetch_row($res_c_min);
					$tiempo_c_min = $reg_c_min[0];
				...
				for ($j = 1; $j < $nro_escogidos + 1 ; $j++) {
						...
						while ($reg_hor=mysql_fetch_row($res_hor)) {	
							...
							for ($contador = 0; $contador < $diff + 1; $contador++) {
										...
										$fecha_inicia_incidencia = $date1. " " . $hor_ini_inc;
										$fecha_fin_incidencia = $date1. " ". $hor_fin_inc;
										/*
										...
										if ($validar == 1) {		
												...
												if (!$responseValidacion["error"]) {
														...
														$sql_inc = 
																"INSERT INTO cab_incidencia 
																		(id,cip,fec_reg,usu_reg,tp_incidencia,motivo_incidencia,fec_ini_inc,fec_fin_inc,obs_incidencia,nro_participantes,cod_incidencia,ejecuto,tiempo,modo,c_doid,dni,dias,estado_inc,fec_mov_est,usu_mov_est,tt_min) VALUES ('','$reg_ch1[3]',now(),'$iduser','$c_mot_gru','$c_mot_inc_gru','$fecha_inicia_incidencia', '$fecha_fin_incidencia','$observacion','$nro_escogidos', '$c_incidencia','','$tiempo_cal','$modo_gru', '$c_doid_gru','$dni_escogidos[$j]','1','0',now(),'$iduser','$tiempo_c_min')";
														//echo $sql_inc;
				...
				if(count($responseValidacion) > 0){
					...
					for ($i = 1; $i<$nro_escogidos + 1 ; $i++) {
							...
							if ($reg_c0[5]=="") {  // Si gestor se encuentra sin horario								
								...
								//14.10.2025				
								$c_min = "SELECT SUBSTRING_INDEX(SEC_TO_TIME((TIMESTAMPDIFF(MINUTE,
								'$fec_ini_inc', '$fec_fin_inc'))*3600),':',1)";
								$res_c_min = mysql_query($c_min);
								$reg_c_min = mysql_fetch_row($res_c_min);
								$tiempo_c_min = $reg_c_min[0];
							...
							if (!$responseValidacion["error"]) {
								...
								$sql_inc=
											"INSERT INTO cab_incidencia
													(id,cip,fec_reg,usu_reg,tp_incidencia,motivo_incidencia,fec_ini_inc,fec_fin_inc,obs_incidencia,
													tiempo,modo,cod_incidencia,c_doid,dni,dias,nro_participantes,estado_inc,fec_mov_est,usu_mov_est,tt_min)
											VALUES	
													('','$cip',now(),'$iduser','$c_mot_gru','$c_mot_inc_gru','$fec_ini_inc','$fec_fin_inc','$observacion',
													'$dif_tiempo','$modo_gru','$c_incidencia',
													'$c_doid_gru','$dni_escogidos[$i]','0','$nro_escogidos','0',now(),'$iduser','$tiempo_c_min')";
											//echo "<br>".$sql_inc;	
												...
												if ($validar=="1") {
														...
														if (!$responseValidacion["error"]) {
																...
																$sql_inc = "INSERT INTO cab_incidencia
																			...
																			'$c_doid_gru','$dni_escogidos[$j]','1','0',now(),'$iduser')";
																	//echo $sql_inc;
																	$res_sql 	= mysql_query($sql_inc);
														}
												}// if validar
...
if ($accion=="grabar_motivos_inc"){
	...
	if ($nreg<1){
		...
		$sql_1 = "SELECT COALESCE(MAX(CONVERT(cod_mot_inc, UNSIGNED)), 0) + 1 AS cod_mot_inc FROM tb_motivos_incidencia where tp_inc='$tipo_inc'";
					
		//echo "<br>".$sql_1;
		$res_1 = mysql_query($sql_1);		
		$reg_1 = mysql_fetch_row($res_1);
				
		$sql_2="insert into tb_motivos_incidencia values('$reg_1[0]','$motivo_inc','$tipo_inc','1','COT-TDP','1')";	
		//echo "<br>".$sql_2;	
		$res_2 = mysql_query($sql_2);
...
if($accion=="detalle_maestra_reniec_gescot"){
	...
	echo "
	  <tr>
		<td class='cabeceras_grid'>APELLIDO PATERNO</td>
		<td class='cabeceras_grid'>APELLIDO MATERNO</td>
		<td class='cabeceras_grid'>NOMBRES</td>
		<td class='cabeceras_grid'>FECHA NACIMIENTO</td> 
		<td class='cabeceras_grid'>SEXO</td> 
	  </tr>";
...
if($accion=="detalle_maestra_tb_dni"){

	$dni=$_GET["dni"]; 
	
	$cc				= "select dni,apellido_paterno,apellido_materno,nombres,sexo, fecha_nacimiento from tb_dni where dni='$dni'";		 
				//echo $cc;
	$res_cc			= mysql_query($cc);
	$num_rows_cc	= mysql_num_rows($res_cc);
	...
	echo "
	  ...
	  if ($num_rows_cc>0){
		...
		echo "			
	 	...
	 	echo "			
		  <tr>
			<td colspan='10' class='caja_texto_pe' align='center'>DNI NO SE ENCUENTRA REGISTRADO EN LA BASE DE DATOS</td>
		  </tr>";	 
	 }	 
	echo "</table>";
}
...
if ($accion=="eliminar_detalle_incidencia"){
    ...
    //$sql="delete from cab_incidencia where id='$id_inc'";
	//echo "<br>".$sql;	

	$sql ="update cab_incidencia set estado_inc='3',fec_cancelado=now(),
	mot_cancelado='Error de registro' where cod_incidencia='$inc' and dni='$dni' and fec_ini_inc='$fecha'";
	$res = mysql_query($sql);		
}
...
if($accion=='obtenerGestores'){
	...
	$queryObtenerGestoresPorSupervisor = "SELECT iduser, ncompleto, dni FROM tb_usuarios WHERE c_supervisor = ".$supervisor." AND estado='HABILITADO' ORDER BY 2";
	}
...
if($accion=='listar_incidencias_cot'){	
	
	$nro_escogidos=$_GET["arr_escogidos"];	
	...
	echo "<table width='100%' border='0'>";
		echo "<tr>";
			...
			echo "<td class='cabeceras_grid'># INC.</td>";
			echo "<td class='cabeceras_grid'>ACUM.MINxDIA</td>";
			echo "<td class='cabeceras_grid'>VER</td>";
			echo "</tr>";
		
		
	for ($i = 0; $i < $nro_escogidos + 1 ; $i++) {
		
		$hor ="select * from horarios_gestores_cot where cip='$reg[4]' group by 2 ";
		echo $hor;
		$res_hor = mysql_query($hor);	
		...
		while($reg_hor=mysql_fetch_row($res_hor)){	
			...
			$conta_d	 = "SELECT COUNT(*),sum(tt_min) FROM cab_incidencia WHERE dni='$hor_gestores[$i]' AND SUBSTR(fec_reg,1,10)='$hoy'";
			//echo $conta_d;
			...
			*/
			echo "<td class='caja_texto_pe r_horario'>$reg_ch2[2]</td>";
			echo "<td class='caja_texto_pe h_entrada'>$reg_ch2[5]</td>";
			echo "<td class='caja_texto_pe h_salida'>$reg_ch2[6]</td>";
			echo "<td class='caja_texto_pe r_dias' style='display:none'>$reg_ch2[7]</td>";
...
if($accion=='muestra_horarios_gestor'){
	$c_gestor = $_GET['c_gestor'];
	$tip = $_GET['tip'];
	
	if ($tip=="5"){
		$c_gestor = explode("-", $c_gestor);
		$c_gestor = $c_gestor[0];
	}else{
		$c_gestor = substr($c_gestor,1,8);
	}
	
	$datos="select ncompleto,dni from tb_usuarios where dni like '%$c_gestor%'";
	$res_datos	= mysql_query($datos);
	$reg_datos	= mysql_fetch_row($res_datos);
	
	
	$hor="select * from horarios_gestores_cot where dni like '%$c_gestor%'";
	//echo $hor;
	$res_hor	= mysql_query($hor);
	$reg_hor	= mysql_fetch_row($res_hor);

	$cobeHoraIniHorario = $reg_hor[8];
	$cobeHoraFinHorario = $reg_hor[9];

	$rdias="select * from horarios_rrhh where cod_horario='$reg_hor[5]'";
	//echo $rdias;
	$res_rdias	= mysql_query($rdias);
	$reg_rdias	= mysql_fetch_row($res_rdias);
	
	//echo $hor."|".$reg_hor[6];
	
	echo "<a class='caja_texto_cb'>";
	echo "GESTOR: ".$reg_datos[0]." - ".$reg_datos[1]."<br>";
	echo "HORARIO: ".$reg_hor[6];

	
	echo "<input name='rango_dias' type='hidden' class='caja_texto_sb' id='rango_dias' value='$reg_rdias[7]' />";
	
	/* cobe */
	echo "<input name='rango_horaIniX' type='hidden' class='caja_texto_sb' id='rango_horaIniX' value='$cobeHoraIniHorario' />";
	echo "<input name='rango_horaFinX' type='hidden' class='caja_texto_sb' id='rango_horaFinX' value='$cobeHoraFinHorario' />";
	/* FIN cobe*/
...
if($accion=="cambiar_pass"){
	$login             = $_GET['login'];
    $c_motivoclave     = $_GET['c_motivoclave'];
	$n_clave           = trim($_GET['n_clave']);
    
    
    $sql_1="select dni, pass from tb_usuarios where login='$login'";
	//echo $sql_1."<br>";
	$res_1= mysql_query($sql_1);
    $reg_1 = mysql_fetch_row($res_1);	
    
    $msn               = "La nueva clave es: ".$n_clave."| La clave anterior fue: ".$reg_1[1] ;
    
    $sql_2="INSERT INTO movimiento_usuarios(id,proceso,usu_mov,fec_mov,obs,pc,dato)
	VALUES(LAST_INSERT_ID(),'$c_motivoclave','$iduser',now(),'$msn','$pc','$reg_1[0]')";
	//echo $sql_2."<br>";
	$res_2= mysql_query($sql_2);
    
	$datos="update tb_usuarios set pass='$n_clave',idsess='' where login='$login' ";
    //echo $datos;
	$res_datos	= mysql_query($datos);
...
if ($accion=="actualizar_incidencias_cot"){
  $pc			       = gethostbyaddr($_SERVER['REMOTE_ADDR']);		
	$iduser            =$_GET["iduser"];	
	$c_indicencia      =$_GET["c_indicencia"];
	$tp_incidencia     =$_GET["tp_incidencia"];
	$mot_incidencia    =$_GET["mot_incidencia"];
	$combo_modalidad   =$_GET["combo_modalidad"];	
	$tiempo_inc        =trim($_GET["tiempo_inc"]);
	$tiempo_minutos    =$_GET["tiempo_minutos"];	
	$obs_inc           =trim($_GET["obs_inc"]);
	$fec_ini           =$_GET["fec_ini"];
	$fec_fin	         =$_GET["fec_fin"];	
	$idperfil	         =$_GET["idperfil"];	
  $estado	           ="0";	
    ...
    $sql_act    ="update cab_incidencia set 
    tp_incidencia='$tp_incidencia',
    motivo_incidencia='$mot_incidencia',
    modo='$combo_modalidad',
    tiempo='$tiempo_inc',
    tt_min='$tiempo_minutos',
    obs_incidencia='$obs_inc',
    fec_ini_inc='$fec_ini',
    fec_fin_inc='$fec_fin',
    estado_inc = '$estado',
    usu_reg='$iduser',
    fec_mov_est=now()
    where cod_incidencia='$c_indicencia'
    ";
    //echo $sql_act;
...
if($accion=='mostrar_carga_horarios'){
    
    $archivo    = "D:/xampp/htdocs/GESCOT/CARGAS/INPUT/octubre.csv";
	$sw	        = $_GET["sw"];	
	
	if  ($sw==1){
		$contar		= "SELECT f_carga,count(*) FROM horarios_gestores_cot group by 1";	
		//echo $contar;	
		...
		$nreg_contar = mysql_num_rows($res_contar);
      	...
      	if ($nreg_contar>0){
            ...
            echo "<td>".$archivo;
		  	...
		  	echo "<img src='../image/vis.jpg' width='20' height='20' /></td>";
            echo "</tr>";
            
            echo "<tr>";        
            echo "<td>Ult. Fecha de Carga:</td>";
            echo "<td>";
            
            if (file_exists($archivo)) {
						$f_carga_file=date("Y m d H:i:s", filemtime($archivo));
						echo $f_carga_file."<img src='../image/vis.jpg' width='20' height='20' /></td>";;
				    }else{
                      echo"No se encontro el archivo....";
                      echo "<img src='../image/eliminar.jpg' width='20' height='20' /></td>";
                  } 
            echo "</tr>";
        
            echo "<tr>";        
            echo "<td>Msn:</td>";
            echo "<td>Se cargaron $reg_contar[1] registros correctamente <img src='../image/vis.jpg' width='20' height='20' /></td>";  
            echo "</tr>
            ...
            <tr>";        
            echo "<td>Ult. Fecha de Carga:</td>";
            echo "<td>__/__/____ <img src='../image/eliminar.jpg' width='20' height='20' /></td>";
            echo "</tr>";
        
            echo "<tr>";        
            echo "<td>Msn:</td>";
            echo "<td>No se encontro archivo actualizado <img src='../image/eliminar.jpg' width='20' height='20' /></td>";  
            echo "</tr>
		</table>";
	}
   }
...
if($accion=='actualizar_horarios_gestor'){

	$iduser 	= $_GET['iduser'];
	$dni 		= $_GET['dni'];
	$hor_ant 	= $_GET['hor_ant'];
	$hor_nuevo 	= $_GET['hor_nuevo'];
	$obs_hor 	= $_GET['obs_hor'];
	

	$sql_1      ="update horarios_gestores_cot set cod_horario='$hor_nuevo' where dni='$dni' ";
	echo $sql_1."<br>";
	$res_sql1	= mysql_query($sql_1);
	
	$sql_2="insert into historico_cambio_horarios(gestor,hor_ant,hor_nuevo,iduser,fec_mov,obs) 
	values('$dni','$hor_ant','$hor_nuevo','$iduser',now(),'$obs_hor')";	
	echo $sql_2;
	$res_sql2	= mysql_query($sql_2);
...
if($accion=='mostrar_historico_horarios'){

	$dni 	= $_GET['dni'];

	$hist ="select * from historico_cambio_horarios where gestor='$dni'";
		//echo $hor;
		$res_hist = mysql_query($hist) or die (mysql_error());
	
		echo "<table border='0' class='marco_tabla' width='95%' align='center'>";
				echo "<tr>";
				echo "<td class='caja_texto_peq'>ITEM</td>";
				echo "<td class='caja_texto_peq'>GESTOR</td>";
				echo "<td class='caja_texto_peq'>HOR.ANTERIOR</td>";
				echo "<td class='caja_texto_peq'>HOR.NUEVO</td>";
				echo "<td class='caja_texto_peq'>USU.ACTUALIZA</td>";
				echo "<td class='caja_texto_peq'>FEC.ACTUALIZACION</td>";
				echo "<td class='caja_texto_peq'>OBSERVACION</td>";
			...
			while($reg_hist=mysql_fetch_row($res_hist)){	
					...
					$gestor="select * from tb_usuarios where dni like '%$reg_hist[1]%'";
					//echo $hor;
					$res_gestor	= mysql_query($gestor);
					$reg_gestor	= mysql_fetch_row($res_gestor);

					$usu="select * from tb_usuarios where iduser='$reg_hist[4]'";
					//echo $hor;
				...
				echo "<tr>";
				echo "<td class='caja_texto_mini'>$i</td>";
				echo "<td class='caja_texto_mini'>$reg_gestor[1]</td>";
				echo "<td class='caja_texto_mini'>$reg_hist[2]</td>";
				echo "<td class='caja_texto_mini'>$reg_hist[3]</td>";
				echo "<td class='caja_texto_mini'>$reg_usu[1]</td>";
				echo "<td class='caja_texto_mini'>$reg_hist[5]</td>";	
				echo "<td class='caja_texto_mini'>$reg_hist[6]</td>";	
				echo "</tr>";
				}		
		echo "</table>";
