...
if ($_GET["xmes"]=="0"){
	$cri=substr($_GET["xano"],2,2);	
	$cad="SUBSTR(FECHA_REG,7,2)='$cri'";
}else{
	$cri=$xmes."/".substr($_GET["xano"],2,2);
 	$cad="SUBSTR(FECHA_REG,4,5)='$cri'";
}
...
$lista="(select CIUDAD,PEDIDO,INSCRIPCION,DIRECCION,DISTRITO,PROMOCION,GRUPO,NRO_REF,FECHA_REG,TIPO_SERVICIO,
PRIORIDAD,OFIC_COM,PETICION,AGRUPACION,CLIENTE,CUENTA,PC,NEGOCIO,PREFIJO,SEGMENTO,CANTIDAD,SUB_SEGMENTO,IRIS,
DIRECCION_ANTIGUA,SECTOR,MANZANA,COORD_X,COORD_Y from tb_gestel_423_tot WHERE $cad order by fecha_reg)
union
(select CIUDAD,PEDIDO,INSCRIPCION,DIRECCION,DISTRITO,PROMOCION,GRUPO,NRO_REF,FECHA_REG,TIPO_SERVICIO,
PRIORIDAD,OFIC_COM,PETICION,AGRUPACION,CLIENTE,CUENTA,PC,NEGOCIO,PREFIJO,SEGMENTO,CANTIDAD,SUB_SEGMENTO,IRIS,
DIRECCION_ANTIGUA,SECTOR,MANZANA,COORD_X,COORD_Y from tb_gestel_423_tot_2018 WHERE $cad order by fecha_reg)";
//echo $lista;
