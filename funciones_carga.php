...
function carga_423(){
	...
	echo "<br>"."....Proceso de Carga Gestel 423 OK";
}
...
function exclusiones_423(){		
		...
		*/
		...
		$sql_14a="INSERT IGNORE INTO cab_gestion_actividades_cot
		SELECT peticion,pedido,inscripcion,'','',direccion,
		CONCAT('20',SUBSTR(fecha_reg,7,2),'-',SUBSTR(fecha_reg,4,2),'-',SUBSTR(fecha_reg,1,2))
		AS f_legado,'0' AS estado,'GESTEL-423' AS fuente,'ASIGNACIONES' AS actividad,'' AS marca_1,'' AS marca_2,NOW() AS fec_carga,'' AS zonal,'' AS obs
		FROM tb_gestel_423 WHERE SUBSTR(fec_carga,1,10)=DATE(NOW());";
		//echo "<BR>".$sql_14;
		$res_sql_14a = mysql_query($sql_14a);	
		
		$sql_15a="UPDATE cab_gestion_actividades_cot a, cab_asignaciones_cot b
		SET a.estado=b.estado_asig 
		WHERE a.peticion=b.peticion AND a.actividad='ASIGNACIONES'";
		//echo "<BR>".$sql_3;
		$res_sql_15a = mysql_query($sql_15a) or die(mysql_error($sql_15a));	
				
		$sql_16a="UPDATE cab_gestion_actividades_cot a,  tb_gestel_423_prov b
		SET a.zonal=b.zonal
		WHERE a.peticion=b.peticion and a.fuente='GESTEL-423' AND a.actividad='ASIGNACIONES'";
		//echo "<BR>".$sql_3;
		$res_sql_16a = mysql_query($sql_16a) or die(mysql_error($sql_16a));	
					
		$sql_18a="UPDATE cab_gestion_actividades_cot SET zonal='LIM' WHERE zonal='' and fuente='GESTEL-423' AND actividad='ASIGNACIONES'";
		//echo "<BR>".$sql_3;
		$res_sql_18a = mysql_query($sql_18a) or die(mysql_error($sql_18a));	
		
		/*	
		$sql_19a="delete from cab_gestion_actividades_cot where peticion in ('',' ','N')";
		//echo "<BR>".$sql_3;
		$res_sql_19a = mysql_query($sql_19a) or die(mysql_error($sql_19a));	
		*/
		
		$sql_20a="INSERT INTO ORIGEN_ZONAL
		SELECT TRIM(ZONAL),fuente,estado,COUNT(*) FROM cab_gestion_actividades_cot GROUP BY 1,2,3 ORDER BY 1";
		//echo $junta2;
		$res_sql_20a= mysql_query($sql_20a)or die(mysql_error($sql_20a));	
...
function blanquear_tablas_cms(){
	...
	echo "<br>"."....Proceso Blanquear tabla CMS OK";
}
...
function carga_cms($cms){	
	...
	while (!feof($fp) ) {
		...
		$Nivel_Ubic				=$linea[12];
		$Peticion				=$linea[13];
		$MultiServicio			=$linea[14];
		$Tipo_Deco				=$linea[15];
		$T_Rq					=$linea[16];
		$Motv					=$linea[17];
		$Descripcion_Motivo		=$linea[18];
		$Cod_Cmts				=$linea[19];
		$Descrip_Cmts			=$linea[20];
		$Nodo					=$linea[21];
		$Plano					=$linea[22];
		$Troncal				=$linea[23];
		$Sector					=$linea[24];
		...
		$Dist					=$linea[30];
		$Via					=$linea[31];
		$Direccion				=quitar_tildes($linea[32]);
		$Numero					=$linea[33];
		$INT					=$linea[34];
		$Piso					=$linea[35];
		$Mzn					=$linea[36];
		$Lote					=$linea[37];
		$Urb					=$linea[38];
		$Nombre_Urb				=quitar_tildes($linea[39]);
		$Referencia				=quitar_tildes($linea[40]);
		$Clase_Srv				=quitar_tildes($linea[41]);
		$Señal_de_la_Troba		=$linea[42];
		$_Cr					=$linea[43];
		$Premium				=$linea[44];
		$N_Cross				=$linea[45];
		$Categoria_Srv			=$linea[46];
		$Tlf					=$linea[47];
		
		if ($linea[47]!=""){
			$Fecha_Llegada=$linea[49];
			$Sit="";
		}else{
		...
		$Edo					=$linea[50];
		$Motv_					=$linea[51];
		$Ofc_Adm				=trim($linea[52]);
		$F_Prg_MM				=$linea[53];
		$Prior					=$linea[54];
		$Observaciones			=$linea[55];
		$Autorizacion			=$linea[56];
		$Cod_Autorizacion		=$linea[57];
		$Contacto_Clte			=$linea[58];
		$Cliente_Conforme		=$linea[59];
		$Tel_Contacto_CCT		=$linea[60];
		$Tel_Contacto_REF_CCT	=$linea[61];
		$Mot_Autorizacion		=$linea[62];
		$DescMot_Autorizcion	=$linea[63];
		$Escenario_Autorizacion	=$linea[64];
		$Tecnico				=$linea[65];
		$Nombre_Tecnico			=$linea[66];
		$Fec_Autorizacion		=$linea[67];
		$Usu_Autorizacion		=$linea[68];
		$Encuesta				=$linea[69];
		$Contacto_Enc			=$linea[70];
		$Parentesco_Enc			=$linea[71];
		$Telefono_Enc			=$linea[72];
		$Incidencia				=$linea[73];
		$Ticket_Incidencia		=$linea[74];
		$Seg_Incidencia			=$linea[75];
		$Resegmentacion			=$linea[76];
		$Tipo_Paquete			=$linea[77];
		$N_OS					=$linea[78];
		$Estado_OS				=$linea[79];
		$Tipo_Linea				=$linea[80];
		$NroTlfVOIP				=$linea[81];
		$PromLinea				=$linea[82];
		$Descripcion_Prom_Linea	=$linea[83];
		$Tipo_Tecnologia		=$linea[84];
		$zon					=$linea[85];
		$zon1					=$linea[86];
		$est					=$linea[87];
	...
	*/
	...
	$sql_0="INSERT INTO tb_cms(col,n_requer,Area_Ant,AREA_,Cliente,VIP,CORP,Agendado,En_Gaudi,Seg_Gaudi,PAI,mov_total,
...
'$Tipo_Deco','$T_Rq','$Motv','$Descripcion_Motivo','$Cod_Cmts','$Descrip_Cmts','$Nodo','$Plano','$Troncal','$Sector','$Line_Ext','$Tap','$Borne','$Numcoo_X','$Numcoo_Y','$Dist','$Via','$Direccion','$Numero','$INT','$Piso','$Mzn','$Lote','$Urb','$Nombre_Urb','$Referencia','$Clase_Srv','$Senal_de_la_Troba','$_Cr','$Premium','$N_Cross','$Categoria_Srv','$Tlf','$Fecha_Llegada','$Sit','$Edo','$Motv_','$Ofc_Adm','$F_Prg_MM','$Prior','$Observaciones','$Autorizacion','$Cod_Autorizacion','$Contacto_Clte','$Cliente_Conforme','$Tel_Contacto_CCT','$Tel_Contacto_REF_CCT','$Mot_Autorizacion','$DescMot_Autorizcion','$Escenario_Autorizacion','$Tecnico','$Nombre_Tecnico','$Fec_Autorizacion','$Usu_Autorizacion','$Encuesta','$Contacto_Enc','$Parentesco_Enc','$Telefono_Enc','$Incidencia','$Ticket_Incidencia','$Seg_Incidencia','$Resegmentacion','$Tipo_Paquete','$N_OS','$Estado_OS','$Tipo_Linea','$NroTlfVOIP','$PromLinea','$Descripcion_Prom_Linea','$Tipo_Tecnologia','$zon','$zon1','$est',now());";
	...
	*/
	...
	/*********************************************************************************/
		...
		$sql_7a="INSERT ignore INTO cab_gestion_actividades_cot
		SELECT n_requer,peticion,'' AS inscr,cliente,'' AS telf,CONCAT(via,',',TRIM(direccion),urb,',',nombre_urb,'-',referencia)
		AS  direccion,CONCAT(SUBSTR(fecha_llegada,7,4),'-',
		SUBSTR(fecha_llegada,4,2),'-',SUBSTR(fecha_llegada,1,2),' ',
		SUBSTR(fecha_llegada,12,5)) AS fec_legado,'0' AS estado,'CMS' AS fuente,'ASIGNACIONES','' AS marca_1,'' AS marca_2,NOW() AS fec_carga,TRIM(zon) AS zonal,'' AS obs
		FROM tb_cms ORDER BY 7";
		//echo "<BR>".$sql_7a;
		$res_sql_7a = mysql_query($sql_7a) or die(mysql_error($sql_7a));
		
		
		$sql_8a="UPDATE cab_gestion_actividades_cot a, cab_asignaciones_cot b
			SET a.estado=b.estado_asig 
			WHERE a.requerimiento=b.peticion AND a.actividad='ASIGNACIONES' AND a.fuente='CMS'";
		//echo "<BR>".$sql_8a;
		$res_sql_8a = mysql_query($sql_8a) or die(mysql_error($sql_8a));
		
		
		/*********************************************************************************/	
	
	echo "<br>"."....Proceso de Carga CMS OK";	
	}
...
function carga_tabla_GESTEL_47D($fileName,$zon){	
	...
	while (!feof($gestel_47D) ) {
				...
				$Motivo = trim(substr($linea, 69, 100));
				...
				*/
				...
				$sql_1="INSERT ignore INTO gestel_47d (Fecha,Comprob_,Inscripcion,Solicitud,Usuario,Cod_Sol,Motivo,Pedido,Red,MDF,Cabl ,PAli ,Armar ,Bloq ,PDis ,
				Caja ,Born ,MDFd,Cabld ,PAlid ,Armard ,Bloqd ,PDisd ,Cajad ,Bornd ,Telefono ,Sect ,Manz,Peticion ,Agrupacion ,
				Cliente,Cuenta ,PC,Tipo_Circuito1,N_Circuito1,Tipo_Circuito2,N_Circuito2,Segmento,origen,fec_carga,n_area) 
...
function llenar_tablas_contactos($fileName,$tabla){
		...
		$row=1;		
		echo "<br>2.- PROCESO LLENADO DE TABLAS EJECUTANDOSE INICIADO ";
...
function cruces_tabla_contactos($tabla){
	...
	$sql_4		= "delete from $tabla where campo3 in ('','-')";		
	//echo "<br>".$sql;
	...
	$sql_3a		= "update $tabla set pedido=trim(pedido)";		
	//echo "<br>".$sql;
	$res_3a = mysql_query($sql_3a, $link) or die(mysql_error($sql_3a));
		
	
	$sql_4		= "update $tabla set campo4=trim(campo4)";		
	//echo "<br>".$sql;
	$res_4 = mysql_query($sql_4, $link) or die(mysql_error($sql_4));
	
	$sql_5		= "update $tabla set campo5=trim(campo5)";	
	//echo "<br>".$sql;
	$res_5 = mysql_query($sql_5, $link) or die(mysql_error($sql_5));
	
	$sql_6		= "update $tabla set campo6=trim(campo6)";		
	//echo "<br>".$sql;
	$res_6 = mysql_query($sql_6, $link) or die(mysql_error($sql_6));
	
	$sql_7		= "update $tabla set campo7=concat(trim(campo4),' ',trim(campo5),', ',trim(campo6))";		
	//echo "<br>".$sql;
	$res_7 = mysql_query($sql_7, $link) or die(mysql_error($sql_7));
	
	$sql_8		= "UPDATE $tabla SET campo7='' WHERE campo4=''";		
	//echo "<br>".$sql;
	$res_8 = mysql_query($sql_8, $link) or die(mysql_error($sql_8));
...
function cargar_load_atendidas(){
		
	$link = mysql_connect("localhost", "root", "") ;
	$db = mysql_select_db("cot", $link) ; 
	
	//A)
	$tabla1 	="tdata_expsitped_atis";
	$archivo1	="D:/compartido/data/canceladas/SITPED.csv";
	
	mysql_query("truncate table $tabla1");
	
	$pre_canceladas_1="LOAD DATA LOCAL INFILE '$archivo1'
	INTO TABLE $tabla1
	FIELDS TERMINATED BY ','
	LINES TERMINATED BY '\n' IGNORE 1 LINES";
	//echo "<br>".$paso_WC1;
	$res_precan1 = mysql_query($pre_canceladas_1, $link) or die(mysql_error());		
	
	//B)
	
	$tabla2 	="tdata_expsitped_cms";
	$archivo2	="D:/compartido/data/canceladas/SITPEDCMS.csv";
	
	mysql_query("truncate table $tabla2");
	
	$pre_canceladas_2="LOAD DATA LOCAL INFILE '$archivo2'
	INTO TABLE $tabla2
	FIELDS TERMINATED BY ','
	LINES TERMINATED BY '\n' IGNORE 1 LINES";
	//echo "<br>".$paso_WC1;
	$res_precan2 = mysql_query($pre_canceladas_2, $link) or die(mysql_error());		
	
	echo "<br>"."....Carga de Tablas Cms y Atis BI OK";

}

function cargar_load_canceladas($fileName){
	
	
	$link = mysql_connect("localhost", "root", "") ;
	$db = mysql_select_db("cot", $link) ; 
	
	mysql_query("truncate table 1_canceladas_sinweb");
	
	$pre_canceladas_1="LOAD DATA LOCAL INFILE '$fileName'
	INTO TABLE 1_canceladas_sinweb
	FIELDS TERMINATED BY ','
	LINES TERMINATED BY '\n' IGNORE 1 LINES ";
	//echo "<br>".$pre_canceladas_1;
	$res_precan1 = mysql_query($pre_canceladas_1, $link) or die(mysql_error());		
	
	$pre_canceladas_2="UPDATE 1_canceladas_sinweb SET 
	fecreg=CONCAT(SUBSTR(fecreg,7,4),'-',SUBSTR(fecreg,4,2),'-',SUBSTR(fecreg,1,2)),
	fpai_reg=CONCAT(SUBSTR(fpai_reg,7,4),'-',SUBSTR(fpai_reg,4,2),'-',SUBSTR(fpai_reg,1,2)),
	fgestion=CONCAT(SUBSTR(fgestion,7,4),'-',SUBSTR(fgestion,4,2),'-',SUBSTR(fgestion,1,2))";	
	//echo "<br>".$paso_5;
	$res_precan2 = mysql_query($pre_canceladas_2,$link) or die(mysql_error());		
	
	
	$pre_canceladas_3="UPDATE 1_canceladas_sinweb set estado_act='PENDIENTE'";	

	//echo "<br>".$paso_5;
	$res_precan3 = mysql_query($pre_canceladas_3,$link) or die(mysql_error());	
	
	
	
	$pre_canceladas_3="UPDATE 1_canceladas_sinweb a, tdata_expsitped_atis b 
	SET a.estado_act=CONCAT('ENCONTRO-ATIS')
	WHERE a.pet_req=b.peticion";	

	//echo "<br>".$paso_5;
	$res_precan3 = mysql_query($pre_canceladas_3,$link) or die(mysql_error());	
	
	$pre_canceladas_4="UPDATE 1_canceladas_sinweb a, tdata_expsitped_cms b 
	SET a.estado_act=CONCAT('ENCONTRO-CMS')
	WHERE a.pet_req=b.CODREQ AND a.estado_act='PENDIENTE'";	

	//echo "<br>".$paso_5;
	$res_precan4 = mysql_query($pre_canceladas_4,$link) or die(mysql_error());		
		
	echo "<br>"."....Carga de Canceladas OK";
}

function cargar_canceladas_pedidos(){

	$hoy = date("Y-m-d");
	
	$pro_canceladas_1="INSERT ignore INTO cab_gestion_actividades_cot
	SELECT pet_req,'','','','','',fecreg,'PENDIENTES','WEB ASEGURAMIENTO','CANCELADAS','','',NOW(),CZONAL,''
	FROM 1_canceladas_sinweb WHERE estado_act='PENDIENTE'";	
	//echo "<br>".$paso_3;
	$res_pcan1 = mysql_query($pro_canceladas_1);
	
	$pro_canceladas_2="UPDATE  cab_gestion_actividades_cot SET marca_1='REQUERIMIENTOS' WHERE SUBSTR(requerimiento,1,1) IN (9,6,1) AND actividad='CANCELADAS'";
	//echo "<br>".$paso_4;
	$res_pcan2 = mysql_query($pro_canceladas_2) or die(mysql_error($pro_canceladas_2));
		
	$pro_canceladas_3="UPDATE  cab_gestion_actividades_cot SET marca_1='PETICIONES' WHERE actividad='CANCELADAS' AND marca_1=''";
	//echo "<br>".$paso_3;
	$res_pcan3 = mysql_query($pro_canceladas_3) or die(mysql_error($pro_canceladas_3));
		
	$pro_canceladas_4="UPDATE cab_gestion_actividades_cot a, cancelaciones_trabajadas b
	SET a.estado=b.est,a.marca_2=b.pc
	WHERE a.requerimiento = b.codigo AND SUBSTR(b.fec_eje,1,10)='$hoy' AND a.actividad='CANCELADAS'";
	//echo "<br>".$paso_3;
	$res_pcan4 = mysql_query($pro_canceladas_4) or die(mysql_error($pro_canceladas_4));
	
	echo "<br>"."....Carga de Canceladas a la tabla pedidos OK";
...
function cargar_load_migraciones($fileName){

	mysql_query("truncate table tb_migraciones");
	
	$link = mysql_connect("localhost", "root", "") ;
	$db = mysql_select_db("cot", $link) ; 
	
	$paso_1="LOAD DATA LOCAL INFILE '$fileName' 
	INTO TABLE tb_migraciones FIELDS ESCAPED BY '\n' TERMINATED BY ',' ENCLOSED BY '' LINES TERMINATED BY '\r\n' IGNORE 1 LINES ";
	//echo "<br>".$paso_WC1;
	$res_1 = mysql_query($paso_1, $link) or die(mysql_error());		

	echo "<br>"."....Carga de Migraciones OK";

}


function cargar_migraciones_pedidos(){

	$hoy = date("Y-m-d");
		
	$paso_1="INSERT INTO cab_gestion_actividades_cot
	SELECT telefono,'0',codservcms,codclicms,telefono,CONCAT(region,'|',jefatura_cmr) AS direccion,'' f_legado,
	'PENDIENTES' AS estado,'REPORTE MIGRACIONES' AS fuente,'MIGRACIONES',grupo_migra,migracion,CURDATE() AS dia,jefatura_cmr,''
	FROM tb_migraciones";	
	//echo "<br>".$paso_3;
	$res_1 = mysql_query($paso_1);	
	
	echo "<br>"."....Carga de Migraciones a Tabla pedidos OK";
...
function carga_formatopsi(){

$paso_0="TRUNCATE TABLE formato_psi";	
//echo "<br>".$paso_0;
$res_0 = mysql_query($paso_0);	

unlink('D:/COMPARTIDO/DATA/PSI/asignaciones_psi_'.date("mY").'.csv');
...
'' AS EMBAJADORCORREO,'' AS EMBAJADORCELULAR,'' AS EMBAJADORDNI,'' AS COMENTARIO,FECHA_LLEGADA AS FH_REG104,NOW() AS FH_REG1L,'' AS FH_REG2L,n_requer AS CODMULTIGESTION,
...
$archivo_out="D://COMPARTIDO/DATA/PSI/asignaciones_psi_".date("mY").".csv";
...
function copiar_psi(){
	...
	$archivo_local = 'D://COMPARTIDO/DATA/PSI/'.$archivo; //Nombre archivo en nuestro PC
