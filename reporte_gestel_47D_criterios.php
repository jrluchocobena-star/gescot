...
if ($_GET["xmes"]=="0"){
	$cri=$xano;	
	$cad="SUBSTR(fecha,1,4)='$cri'";
}else{
	$cri=$xano."-".$xmes;
 	$cad="SUBSTR(fecha,1,7)='$cri'";
}
...
$lista="(select * from gestel_47d_total where $cad group by Comprob_ order by fecha)union
(select * from gestel_47d_total_2018 where $cad group by Comprob_ order by fecha)";
//echo $lista;
