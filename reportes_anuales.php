...
for($i = 0; $i < 6; $i++) {
	...
	if ($i == 1){
	echo "1.- Inicio de proceso: Reportes_anuales.php..."."\n";
	}
	...
	copiar_reporteincidencia_teradata();	
	}
...
function generar_reporte_maestra_teradata(){
		...
		$sql_data	="delete from DBI_COT.CC_COT_MAESTRAUSUARIOS";			
		
		$statementx =odbc_exec($connection_teradata, $sql_data);
		if (!$statementx){
		...
		$rs_paso_16 = mysql_query($paso_16,$connection_mysql);
		...
		$sql_tdata_2	="INSERT INTO DBI_COT.CC_COT_MAESTRAUSUARIOS
		('$con','$rg_paso_16[3]','$rg_paso_16[2]','$rg_paso_16[2]','$rg_paso_16[1]','$rg_paso_16[12]','$rg_paso_16[13]',
		'$rg_paso_16[14]','$rg_paso_16[9]','','','','$rg_paso_16[28]','$edad','','','$rg_paso_16[37]','$rg_paso_16[37]',
		'$reg_d1[3]','$reg_d1[1]','$rg_paso_16[18]','$rg_paso_16[39]','$reg_d2[1]',now(),'$rg_paso_16[0]')";
		//echo "<p>".$sql_tdata_2;
		
		
		$statement_2 =odbc_exec($connection_teradata, $sql_tdata_2);
		if (!$statement_2){
			exit("Execution failed - ".odbc_error().":".odbc_errormsg()."\n");
		}
	}
...
tiempo,modo,c_doid,dni,dias,SUBSTR(fec_ini_inc,1,7),'','','','',now()  
FROM cab_incidencia 
WHERE estado_inc IN('0','1')
ORDER BY fec_reg DESC
)union(SELECT id,TRIM(cip),'' AS cod_horario,'' AS horario,'','',fec_reg,usu_reg,tp_incidencia,trim(motivo_incidencia),fec_ini_inc,fec_fin_inc,obs_incidencia,nro_participantes,cod_incidencia,ejecuto,trim(tiempo),modo,c_doid,dni,dias,SUBSTR(fec_ini_inc,1,7),'','','','',now()  
...
function copiar_reporteincidencia_teradata(){
		...
		$rs_paso_15 	= mysql_query($paso_15,$connection_mysql);

		while($rg_paso_15=mysql_fetch_row($rs_paso_15)){			
		
	
		$sql_tdata_1	="INSERT INTO DBI_COT.CC_COT_INCIDENCIAS
		('$rg_paso_15[0]','$rg_paso_15[1]','$rg_paso_15[2]','$rg_paso_15[3]','$rg_paso_15[4]','$rg_paso_15[5]','$rg_paso_15[6]',
		'$rg_paso_15[7]','$rg_paso_15[8]','$rg_paso_15[10]','$rg_paso_15[11]','$rg_paso_15[14]','$rg_paso_15[16]','$rg_paso_15[17]',
		'$rg_paso_15[18]','$rg_paso_15[19]','$rg_paso_15[20]','$rg_paso_15[21]','$rg_paso_15[22]','$rg_paso_15[23]','$rg_paso_15[24]',
		'$rg_paso_15[25]',now(),'$rg_paso_15[12]','$rg_paso_15[9]')";
		//echo "<p>".$sql_tdata_1;
