...
//echo "usu".$iduser."|cip".$cip."|super".$supervisor."|ini".$fec_ini."|fin".$fec_fin."perfil".$perfil;


/*
$fec_ini=$_GET["fec_ini"];	
$fec_ini_=substr($fec_ini,6,4)."-".substr($fec_ini,3,2)."-".substr($fec_ini,0,2)." ".substr($fec_ini,11,8);

$fec_fin=$_GET["fec_fin"];	
$fec_fin_=substr($fec_fin,6,4)."-".substr($fec_fin,3,2)."-".substr($fec_fin,0,2)." ".substr($fec_fin,11,8);
*/

if ($cip=="" and $_GET["xmes"]=="0" and $supervisor=="0" and $motivo=="0"){
		$mes_actual=date("Y-m");
		$cad="where SUBSTR(fec_ini_inc,1,7)='$mes_actual'";
		
	}else{
		
		if ($cip=="" and $supervisor=="0" and $motivo=="0"){
			$cad="where SUBSTR(fec_ini_inc,1,7)='$xmes'";	
			
		}else{
			if ($supervisor=="0" and $motivo=="0"){
				$cad="where cip like '%$cip%'";			
			}else{	
				if ($motivo=="0"){		
				$cad="where usu_reg='$supervisor'";				
				}else{
				$cad="where motivo_incidencia='$motivo'";					
				}
			}
		}
	}
...
<? 
	...
	$lista="select * from cab_incidencia $cad order by fec_reg desc";
	echo $lista;
	$i=0;
	...
	*/
	while($reg_lis=mysql_fetch_row($res_lis)){		
		...
		?>
        <a href="javascript:edit_incidencia('<? echo $reg_lis[10] ?>','<? echo $reg_lis[4] ?>','<? echo $reg_lis[6] ?>',
        '<? echo $reg_lis[7] ?>','<? echo quitar_tildes($reg_lis[8]) ?>','<? echo $reg_lis[0] ?>','<? echo $reg_lis[4]  ?>')" >        
		<? 
		echo $reg_lis[10]; 				
		echo "</a>";
		echo "</td>";			
		...
		*/
        ...
        <? if ($iduser=="156"){ ?>
        <img src="image/eliminar3.jpg" width="10" height="10" onclick="javascript:borrar_incidencia('<? echo $reg_lis[0] ?>');" title="Eliminar" />	
		<?
		}
        echo "</td>";
