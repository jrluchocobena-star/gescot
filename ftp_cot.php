...
$pass_at = '4t3nt0c0t';
$depth_max_at = 1;
$path_at = '/COT/Fuentes_WA';

$server_at=new TmFtpServer($servername_at, $user_at, $pass_at, $path_at);
$server_at->openConnection();

/**
* Eliminar archivos en FTP.
*/

$files = ftp_nlist($server_at->getConnID(), ".");
foreach ($files as $file)
{
    print "Archivo eliminado de FTP: ".$file."\n";
	ftp_delete($server_at->getConnID(), $file);
}

/**
* Cargar archivos devueltas en FTP.
*/

$src_dir='\\\\10.226.5.114\\FS-Principal\\Dir_Marketing\\Ger_Consumo_de_Terminales_y_Banda_Ancha\\Jefatura_de_Terminales\\000Cot\\DEVUELTAS';
$dst_dir=$path_at;
$d = dir($src_dir);
while($file = $d->read()) { // do this for each file in the directory
	if ($file != "." && $file != "..") { // to prevent an infinite loop
		if (!is_dir($src_dir."/".$file)) { // do the following if it is not a directory
			...
			print "Archivo cargado devueltas en FTP: ".$file. "\n";
		}
	}
}
$d->close();
...
/**
* Cargar archivos maestras en FTP.
*/

$src_dir='D:\COMPARTIDO\DATA\MAESTRA';
$dst_dir=$path_at;
$d = dir($src_dir);
while($file = $d->read()) { // do this for each file in the directory
	if ($file != "." && $file != "..") { // to prevent an infinite loop
		if (!is_dir($src_dir."/".$file)) { // do the following if it is not a directory
			$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$file, $src_dir."/".$file, FTP_BINARY); // put the files
			print "Archivo cargado maestra en FTP: ".$file. "\n";
		}
	}
}
$d->close();
...
/**
* Cargar archivos incidencias en FTP.
*/
$src_dir='D:\COMPARTIDO\DATA\INCIDENCIAS';
$dst_dir=$path_at;
$d = dir($src_dir);
while($file = $d->read()) { // do this for each file in the directory
	if ($file != "." && $file != "..") { // to prevent an infinite loop
		if (!is_dir($src_dir."/".$file)) { // do the following if it is not a directory
			$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$file, $src_dir."/".$file, FTP_BINARY); // put the files
			print "Archivo cargado incidencias en FTP: ".$file. "\n";
		}
	}
}
$d->close();
...
/**
* Cargar archivos de Torre de Control.
*/
$src_dir='\\\\gppesvlcli1003\\FS Explotacion\\ReportesBI\\TorreControl';
$dst_dir=$path_at;

//Cargamos archivo VCP
$filevcp='Vcp_despacho.txt';
$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$filevcp, $src_dir."\\".$filevcp, FTP_BINARY); // put the files
print "Archivo cargado Vcp_despacho en FTP: ".$filevcp. "\n";

//Cargamos archivo Liquidacion Online
$fileliqonline='Liquidacion_online.txt';
$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$fileliqonline, $src_dir."\\".$fileliqonline, FTP_BINARY); // put the files
print "Archivo cargado Liquidacion Online en FTP: ".$fileliqonline. "\n";

//Cargamos archivo Online Comercial
$filecomonline='Online_Comercial_total.txt';
$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$filecomonline, $src_dir."\\".$filecomonline, FTP_BINARY); // put the files
print "Archivo cargado Online Comercial en FTP: ".$filecomonline. "\n";

/**
* Cargar archivo EjePen(pendientes.csv)
*/
$src_dir='\\\\10.226.5.114\\FS-Principal\\Dir_Marketing\\Ger_Consumo_de_Terminales_y_Banda_Ancha\\Jefatura_de_Terminales\\000Cot\\zz_InputsExternos\\EjePen';
$fileejepen='pendientes.csv';
$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$fileejepen, $src_dir."\\".$fileejepen, FTP_BINARY); // put the files
print "Archivo cargado EjePen en FTP: ".$fileejepen. "\n";

/**
* Cargar archivos Conformidad Avería.
*/
$dst_dir='/COT/CargasPSI';
$src_dir='\\\\10.226.5.114\\FS-Principal\\Dir_Marketing\\Ger_Consumo_de_Terminales_y_Banda_Ancha\\Jefatura_de_Terminales\\000Cot\\zz_InputsExternos\\WebCms';
$file='indica_catv_pais_'.date('my').'.xls';
$upload = ftp_put($server_at->getConnID(), $dst_dir."/".$file, $src_dir."/".$file, FTP_BINARY); // put the files
print "Archivo cargado para Conformidad Averia en FTP: ".$file. "\n";
