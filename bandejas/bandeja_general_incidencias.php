...
if ($opc=="1"){		
	$sql_1="select * from cab_incidencia 
	where tp_incidencia IN ('CAMBIO DE TURNO','COMPENSACIONES','HORAS EXTRAS','LICENCIAS','PERMISO') $cad 
...
<thead>   
    <tr>
	 <? 
	if ($cri2=="0"){			
		echo "<th colspan='11'>SUPERVISOR: Todos";
	}else{
		if ($opc=="1"){
			$sp="select * from tb_supervisores where cod_supervisor='$cri2'";
		}else{
  ...
  <tr>
			...
			echo "<input name='filter' size='15' maxlength='20' onkeyup='Table.filter(this,this)' value='INC-'></th>";
			
			if ($opc=="1"){				
				echo "<th class='table-filterable table-sortable:default'>REGISTRADO POR</th>";
			}else{
				echo "<th class='table-filterable table-sortable:default'>MONITOR(A)</th>";
			}
			...
			echo "<th class='filterable'>FEC.INICIO";						
			echo "<input name='filter' size='15' maxlength='15' onkeyup='Table.filter(this,this)'></th>";
			...
			echo "<th class='table-filterable table-sortable:default'>TIPOs</th>";
...
<tbody>
		...
		if ($opc=="1"){		
			$sup="select * from tb_supervisores where cod_supervisor='$reg_tra[23]'"; //supervisor
			//  echo $sup;
